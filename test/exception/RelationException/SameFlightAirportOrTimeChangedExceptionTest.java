...
import exception.grammarException.DepOrArrAirportFormatException;
import exception.grammarException.DepOrArrAirportMissingException;
import exception.grammarException.DeparOrArriTimeFormatException;
import exception.grammarException.FlightDateFormatException;
import exception.grammarException.FlightDateMissingException;
import exception.grammarException.FlightIDFormatException;
import exception.grammarException.FlightIdMissingException;
import exception.grammarException.FrameworkFormatException;
import exception.grammarException.PlaneAgeFormatException;
import exception.grammarException.PlaneIdFormatException;
import exception.grammarException.PlaneSeatsFormatException;
import exception.grammarException.PlaneTypeFormatException;
...
public class SameFlightAirportOrTimeChangedExceptionTest {

	//测试策略：
		//调用非法文本文件src/exception/testFiles/Flight17.txt 即同一航班不同日期的起飞到达时间或机场发生变动
		//观察是否会抛出SameFlightAirportOrTimeChangedExceptionTest异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try {
			Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight17.txt"));
		while (fileScan.hasNext()) {
				...
				if(newEn!=null)
				{
					if (newEn.getSlot().getBeginTime().get(Calendar.DAY_OF_YEAR) == firstCal
						...
						if (!(Integer.valueOf(depar[0]) == newEn.getSlot().getBeginTime()//起飞时间应相同
							...
							throw new SameFlightAirportOrTimeChangedException("AirportOrTimeChanged");
						}
						else {//满足条件 就新建一个计划项
			...
			}//AirportOrTimeChanged
		assertTrue(false); //不应执行到这里应该在读取完该错误的文本文件后发现异常	
	}
catch(Exception e)
{
	assertEquals("AirportOrTimeChanged",e.getMessage());
}
}
