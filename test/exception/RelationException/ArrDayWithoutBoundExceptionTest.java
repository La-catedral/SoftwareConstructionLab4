...
public class ArrDayWithoutBoundExceptionTest {

	//测试策略：
	//调用非法文本文件src/exception/testFiles/Flight9.txt 即到达时间与起飞时间相隔超过一天的情况
	//观察是否会抛出ArrDayWithoutBoundException异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try {
			Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight9.txt"));
		while (fileScan.hasNext()) {
			...
			if (m.matches())// 若不匹配
			{
				...
				if (!(firstCal.get(Calendar.DAY_OF_YEAR) == newCal.get(Calendar.DAY_OF_YEAR) - 1)) {//起飞和到达时间超过一天
			...
			}//DepatureTime:2020-01-18 03:51
			}assertTrue(false); //不应执行到这里应该在读取完该错误的文本文件后发现异常	
		}
	catch(Exception e)
	{
		assertEquals("2020-01-18 03:51",e.getMessage());
	}
}
