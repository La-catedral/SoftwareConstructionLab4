...
public class SamePlaneInfoChangedExceptionTest {

	//测试策略：
			//调用非法文本文件src/exception/testFiles/Flight18.txt 即同一飞机信息发生变动
			//观察是否会抛出SamePlaneInfoChangedExceptionTest异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try {
			Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight18.txt"));
		while (fileScan.hasNext()) {
			...
			
			{
				...
				if( thisOne!=null)
				{
					...
					throw new SamePlaneInfoChangedException("PlaneInfoChangedException");
				}
			...
			}//PlaneInfoChangedException
			}assertTrue(false); //不应执行到这里应该在读取完该错误的文本文件后发现异常	
		}
		catch(Exception e)
		{
			assertEquals("PlaneInfoChangedException",e.getMessage());
		}
		}
