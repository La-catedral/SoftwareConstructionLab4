...
public class FlightDateMissingExceptionTest {

	//测试策略：
		//调用非法文本文件src/exception/testFiles/Flight3.txt 存在航班日期缺失的错误
		//观察是否会抛出FlightDateMissingExceptionTest异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try (Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight3.txt"));){
	...
	{
		assertEquals("Flight:,AA811",e.getMessage());
	}
}
