...
public class FrameworkFormatExceptionTest {

	//测试策略：
	//调用非法文本文件src/exception/testFiles/Flight16.txt 存在文件整体框架，也就是元素定义中分量的数目或次序与规范不符合的错误
	//观察是否会抛出FrameworkFormatExceptionTest异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try (Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight16.txt"))){
		while (fileScan.hasNext()) {
			...
			}//Flight:2020-01-11,AA921
			}assertTrue(false); //不应执行到这里应该在读取完该错误的文本文件后发现异常	
		}
	catch(Exception e)
	{
		assertEquals("Flight:2020-01-11,AA921",e.getMessage());
	}
}
