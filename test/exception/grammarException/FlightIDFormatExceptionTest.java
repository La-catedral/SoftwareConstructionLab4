...
public class FlightIDFormatExceptionTest {

	
	//测试策略：
	//调用非法文本文件src/exception/testFiles/Flight4.txt 存在航班ID格式的错误
	//观察是否会抛出FlightIDFormatException异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try (Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight4.txt"));){
		while (fileScan.hasNext()) {
			...
			}//Flight:2020-01-01,AA8
			}assertTrue(false); //不应执行到这里应该在读取完该错误的文本文件后发现异常	
		}
	catch(Exception e)
	{
		assertEquals("AA8",e.getMessage());
	}
}
