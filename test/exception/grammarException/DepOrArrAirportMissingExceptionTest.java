...
public class DepOrArrAirportMissingExceptionTest {


	//测试策略：
	//调用非法文本文件src/exception/testFiles/Flight5.txt 存在起飞或到达机场缺失的错误
	//观察是否会抛出DepOrArrAirportMissingExceptionTest异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try (Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight5.txt"));){
		...
		{
			assertEquals("DepartureAirport:",e.getMessage());
		}
	}
