...
public class DeparOrArriTimeFormatExceptionTest {

	//测试策略：
		//调用非法文本文件src/exception/testFiles/Flight7.txt 存在起飞或到达时间格式的错误
		//观察是否会抛出DeparOrArriTimeFormatException异常 （捕捉异常 观察相关信息）
		
	
	//test strategy:
	//读取包含起飞或离开时间格式错误的文件 ，捕获异常，并观察异常所发送的信息
	...
	public void test() {
		...
		try {
			Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight7.txt"));
		...
		{
			assertEquals("2020-0-16 22:40",e.getMessage());
		}
	}
