...
public class SignDuplicateExceptionTest {


	//测试策略：
	//调用非法文本文件src/exception/testFiles/Flight19.txt 即存在计划项标签相同的情况
	//观察是否会抛出SignDuplicateExceptionTest异常 （捕捉异常 观察相关信息）
	...
	public void test() {
		...
		try {
			Scanner fileScan = new Scanner(new File("src/exception/testFiles/Flight19.txt"));
			...
			}
			{
				...
				if(newEn!=null)
				{
					if (newEn.getSlot().getBeginTime().get(Calendar.DAY_OF_YEAR) == firstCal
						...
						throw new SignDuplicateException("SignDuplicate");
					} else {//若是不同日期的同一编号的航班
		...
		{
			assertEquals("SignDuplicate",e.getMessage());
		}
		}
